...
import numpy as np
from src.cpflow.logdet_estimators import (
...
class DeepConvexFlow(torch.nn.Module):
    ...
    def reverse(
        self, y, max_iter=1000000, lr=1.0, tol=1e-12, x=None, context=None, **kwargs
    ):
        if x is None:
        ...
        error_new = (
            (self.forward_transform(x, context=context)[0] - y).abs().max().item()
        )
