...
def sample_conditional_and_marginal_coverage(
    ...
    radii_true = get_quantile_level_analytically(
        ...
        dimension=Y_dataset.shape[1]
    ).reshape(-1, 1)

    radii_predicted = pushforward_operator.push_y_given_x(y=Y_dataset, x=X_dataset).norm(dim=-1).reshape(1, -1).detach().cpu()
    ...
    wsc_per_level = np.zeros(number_of_quantile_levels)#, dtype=quantile_levels.dtype)
    for i, quantile_level in enumerate(quantile_levels):
        if verbose:
            print(f"Computing WSC for level = {quantile_level}")
        wsc_per_level[i] = wsc_unbiased(reprs=X_dataset, coverages=is_covered[i].numpy(force=True), delta=0.1)

    coverage_per_level = is_covered.numpy(force=True).mean(axis=-1)

    if verbose:
        np.set_printoptions(precision=2)
        print(f"Nominal level:       {quantile_levels.numpy(force=True)} \n"
              f"Empirical coverage:  {coverage_per_level} \n"
              f"Worst-slab coverage: {wsc_per_level}")

    return quantile_levels, coverage_per_level, wsc_per_level
