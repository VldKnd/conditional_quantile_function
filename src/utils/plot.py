...
import torch
import matplotlib
from datasets import BananaDataset, StarDataset
...
def plot_potentials_from_star_dataset(model: PushForwardOperator, number_of_conditional_points:int, number_of_points_to_sample:int, tensor_parameters: dict, dataset_parameters: dict):

    dataset = StarDataset(**dataset_parameters, tensor_parameters=tensor_parameters)
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(20, 10), subplot_kw={'projection': '3d'})
    fig.suptitle('Separated 3D Plots', fontsize=16)
    ...
    X = torch.stack(
    [
        torch.ones(number_of_conditional_points)*0.5,
        torch.linspace(0, 1, number_of_conditional_points),
    ], dim=-1)
    X = X.to(**tensor_parameters)

    y_x_gt = dataset.sample_conditional(n_points=number_of_points_to_sample, X=X)
    z_scatter = X[:, 1:2].unsqueeze(1).repeat(1, number_of_points_to_sample, 1)
    y_0 = y_x_gt[:, :, 0].flatten()
    y_1 = y_x_gt[:, :, 1].flatten()
    z_scatter = z_scatter.flatten()
    ax1.scatter(y_0, y_1, z_scatter, color='blue', marker='o', s=30, alpha=0.2)
    ...
    for j in range(number_of_conditional_points):
        ...
        for i, contour_radius in enumerate(radii):
            ...
            pushforward_of_u = model.push_forward_u_given_x(u, X=X_batch).detach().cpu()
            z_line = torch.full((pushforward_of_u.shape[0], ), X[j][1].item())
...
def plot_quantile_levels_from_star_dataset(model: PushForwardOperator, number_of_points_to_sample:int, tensor_parameters: dict, conditional_value: float, number_of_quantile_levels: int, dataset_parameters: dict):
    dataset = StarDataset(**dataset_parameters, tensor_parameters=tensor_parameters)
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(20, 10), subplot_kw={'projection': '3d'})
    fig.suptitle('Separated 3D Plots', fontsize=16)
    ...
    X = torch.tensor([[conditional_value, conditional_value]])

    y_x_gt = dataset.sample_conditional(n_points=number_of_points_to_sample, X=X)
    z_scatter = X.unsqueeze(1).repeat(1, number_of_points_to_sample, 1)[:, :, 1]
    ...
    for i, contour_radius in enumerate(radii):
        ...
        z_line = torch.full((pushforward_of_u.shape[0], ), conditional_value)
