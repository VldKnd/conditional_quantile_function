...
def plot_quantile_levels_from_dataset(
    ...
    with torch.no_grad():
        for _, contour_radius in enumerate(radii):
            ...
            ground_truth_U_quantiles = (torch.stack([
                contour_radius * torch.cos(pi),
                contour_radius * torch.sin(pi),
            ]).T).to(**tensor_parameters)
            ground_truth_Y_quantiles = dataset.push_u_given_x(u=ground_truth_U_quantiles, x=X_batch).detach().cpu()

            try:
                approximated_U_quantiels = model.push_y_given_x(y=ground_truth_Y_quantiles, x=X_batch)
            except NotImplementedError:
                approximated_U_quantiels = None

            try:
                approximated_Y_quantiels = model.push_u_given_x(u=ground_truth_U_quantiles, x=X_batch)
            except NotImplementedError:
    ...
    for i, (ground_truth_Y_quantiles, approximated_Y_quantiels) in enumerate(zip(
        list_of_ground_truth_Y_quantiles, list_of_approximated_Y_quantiles
    )):
        ...
        if approximated_Y_quantiels is not None:
            ax1.plot(approximated_Y_quantiels[:, 0], approximated_Y_quantiels[:, 1], z_line, color=color, linewidth=2.5, label=label)

        ax1.plot(ground_truth_Y_quantiles[:, 0], ground_truth_Y_quantiles[:, 1], z_line, ":", color=color, linewidth=2.5, label=label)
        

    for i, (ground_truth_U_quantiles, approximated_U_quantiels) in enumerate(zip(
        list_of_ground_truth_U_quantiles, list_of_approximated_U_quantiles
    )):
        color = color_map(i / number_of_quantile_levels)
        label = f'Quantile level {quantile_levels[i]:.2f}'
        if approximated_U_quantiels is not None:
            ax2.plot(approximated_U_quantiels[:, 0], approximated_U_quantiels[:, 1], z_line, color=color, linewidth=2.5, label=label)

        ax2.plot(ground_truth_U_quantiles[:, 0], ground_truth_U_quantiles[:, 1], z_line, ":", color=color, linewidth=2.5, label=label)
