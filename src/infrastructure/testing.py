...
def sample_inverse_quantile_wasserstein2_distance(
    ...
    for _ in wasserstein2_progress_bar:
        ...
        U_approximation = pushforward_operator.push_u_given_x(u=U_dataset, x=X_dataset)
        wasserstein2_metrics.append(wassertein2(Y_dataset, U_approximation))
...
def sample_quantile_l2_distance(
    ...
    for _ in l2_progress_bar:
        ...
        Y_approximation = pushforward_operator.push_u_given_x(u=U_dataset, x=X_dataset)
        l2_distance = torch.norm((Y_approximation - Y_dataset), dim=-1)**2
...
def test_on_synthetic_dataset(
    ...
    try:
        if not exclude_l2_distance:
            inverse_quantile_metrics["l2_distance"] \
        ...
        print("Skipping inverse quantile l2 distance metrics.")
    ...
    try:
        if not exclude_l2_distance:
            quantile_metrics["l2_distance"] = sample_quantile_l2_distance(
                pushforward_operator=pushforward_operator,
                dataset=dataset,
                number_of_samples=1,
                verbose=verbose
        ...
        print("Skipping quantile l2 distance metrics.")
...
def test(
    ...
    if type(dataset) in {
        ...
        return test_on_synthetic_dataset(
            experiment, exclude_wasserstein2, exclude_l2_distance, verbose
        )
    else:
